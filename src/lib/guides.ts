...
export const SOURCE_GUIDES: SourceGuide[] = [
  ...
  },
  {
    ...
    name: 'urlscan.io',
    region: 'intl',
    applyUrl: 'https://urlscan.io/user/apikey/',
    limits: '免费 ~100 次/天；Key 可选（不填也能查，但限额更低）',
    steps: [
      '打开 urlscan.io 注册账号（免费）。',
      '登录后访问 urlscan.io/user/apikey/。',
      '复制 API Key 填入本插件设置（可不填，匿名也能查）。',
    ],
    note: 'Key 可选；不填则以匿名身份查询，限额更低。',
  },
  {
    id: 'threatfox',
    name: 'ThreatFox',
    region: 'intl',
    applyUrl: 'https://auth.abuse.ch/',
    limits: '免费，需注册获取 Auth-Key',
    steps: [
      '打开 auth.abuse.ch 注册账号（免费）。',
      '登录后在 "Your API keys" 页面找到 ThreatFox Auth-Key。',
      '复制 Auth-Key 填入本插件设置。',
    ],
    note: 'abuse.ch 生态（ThreatFox + MalwareBazaar）共用同一个 Auth-Key。',
  },
  {
    id: 'malwarebazaar',
    name: 'MalwareBazaar',
    region: 'intl',
    applyUrl: 'https://auth.abuse.ch/',
    limits: '免费，需注册获取 Auth-Key（与 ThreatFox 共用）',
    steps: [
      '打开 auth.abuse.ch 注册账号（免费）。',
      '登录后在 "Your API keys" 页面找到 MalwareBazaar Auth-Key。',
      '复制 Auth-Key 填入本插件设置。',
    ],
    note: 'abuse.ch 生态（ThreatFox + MalwareBazaar）共用同一个 Auth-Key。',
  },
  {
    ...
    name: 'Censys',
    region: 'intl',
    applyUrl: 'https://search.censys.io/account/api',
    limits: '免费层有查询额度',
    steps: [
      '打开 search.censys.io 注册账号（免费）。',
      '登录后访问 Account → API 页面。',
      '复制 "API ID" 和 "API Secret"，用冒号连接（格式：API_ID:API_SECRET）填入本插件设置。',
    ],
    note: 'Censys 使用 HTTP Basic Auth，需要 API ID 和 API Secret 组合。',
  },
  {
    id: 'greynoise',
    name: 'GreyNoise',
